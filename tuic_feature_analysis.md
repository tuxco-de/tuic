...
**当前 API 接口详情**：
所有接口调用若配置了 `secret`，则必须在请求头中携带 `Authorization: Bearer <secret>` 进行认证。
1. `GET /online`
   - **能力**：获取当前所有在线用户的连接数统计。
   - **返回**：`HashMap<Uuid, usize>`，以 UUID 为键，值为该用户的并发连接总数。
2. `GET /detailed_online`
   - **能力**：获取更详细的在线客户端信息。
   - **返回**：`HashMap<Uuid, Vec<SocketAddr>>`，列出每个 UUID 当前所有连接客户端的真实来源 IP 和端口号。
3. `GET /traffic`
   - **能力**：获取自服务端启动（或上次重置）以来的用户级流量统计。
   - **返回**：包含 `tx` (发送) 和 `rx` (接收) 字节数的 JSON，例如 `{"UUID": {"tx": 1024, "rx": 2048}}`。
4. `GET /reset_traffic`
   - **能力**：获取当前的流量统计数据，并在返回后**立即将所有用户的计数器清零**。
   - **返回**：与 `/traffic` 格式相同，清零前的数据快照。
5. `POST /kick`
   - **能力**：强制踢出（断开）指定用户的所有活跃连接。
   - **参数**：请求体需为包含 UUID 的 JSON 数组 `[ "UUID1", "UUID2" ]`。
